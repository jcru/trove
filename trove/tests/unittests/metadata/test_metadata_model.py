...
from trove.common.exception import MetadataCreationError
...
class TestMetadataModel(TestMetadataBase):
    ...
    def test_metadataentry_create_by_key_value(self):
        key = 'myNewKey'
        newmeta = {key: 'myTestValue'}
        me = MetadataEntry(None, None, key=key, value=newmeta[key],
                           instance_id=self.instance_id)
        result = me.copy()
        metadata = Metadata(self.context, self.instance_id)
        self.assertIn(key, metadata.keys())
        self.assertEqual(newmeta, result)

    def test_metadataentry_failed_create(self):
        self.assertRaises(MetadataCreationError, MetadataEntry, None, None,
                          key='Test')

    def test_metadata_failed_init(self):
        self.assertRaises(NotFound, Metadata, None, None)
